...
def _record_interest_quietly(chat_id: str, username: str, ts_code: str, query: str, intent: str, profile: dict | None = None) -> None:
    try:
...
@tool
def recommend_get_identity(chat_id: str = "local") -> str:
    """回答推荐助手是谁、记住了什么、能做什么。"""
    return _identity_reply(chat_id or "local")


@tool
def recommend_get_user_profile(chat_id: str) -> str:
    """读取用户画像，包括风险偏好、周期、偏好板块和默认推荐数量。"""
    return json.dumps(get_profile(chat_id or "local"), ensure_ascii=False, default=str)
...
- 推荐股票时必须调用 recommend_get_user_profile 和 recommend_get_trader_memory；通常还要调用 recommend_search_stocks。
...
def _run_recommend_loop(user_input: str, chat_id: str, username: str, max_tool_turns: int = 6, progress_callback=None) -> tuple[dict, list[dict], float, bool]:
    llm = _build_recommend_llm()
    profile = apply_inferred_preferences(chat_id or "local", user_input, username)
    result = ReActLoop(
        llm,
        RECOMMEND_TOOLS,
        metadata={"agent_loop": "telegram_recommender", "chat_id": chat_id or "local"},
    ).run(
...
def run_recommend_react_agent(text: str, chat_id: str = "local", username: str = "", progress_callback=None) -> dict:
    raw = (text or "").strip()
    ...
    if _is_simple_recommend_query(query):
        return _run_rule_recommendation(query, chat_id or "local", username, context, "rule", progress_callback=progress_callback)
    tool_trace: list[dict] = []
    rate = _check_recommend_rate_limit(chat_id or "local")
    if not rate.get("ok"):
        ...
        return result
    try:
        data, tool_trace, latency_ms, json_ok = _run_recommend_loop(query, chat_id or "local", username, progress_callback=progress_callback)
...
def handle_text_message(text: str, chat_id: str = "", username: str = "", progress_callback=None) -> str:
    """Parse a Telegram text message and return a plain text response."""
    raw = (text or "").strip()
    lower = raw.lower()
    profile = apply_inferred_preferences(chat_id or "local", raw, username) if chat_id else None
    if _record_text_feedback(chat_id or "local", username, raw):
        if any(token in raw for token in ("太激进", "太冒险", "恐高")):
            update_profile(chat_id or "local", {"risk_level": "低"}, username)
        elif any(token in raw for token in ("太保守", "机会少")):
            update_profile(chat_id or "local", {"risk_level": "高"}, username)
    ...
    if single_stock_intent:
        ...
        for code in codes[:3]:
            _record_interest_quietly(chat_id or "local", username, code, raw, "single_stock_analysis", profile)
        if progress_callback:
    ...
    if lower.startswith("/profile") or raw.startswith("画像"):
        if "set" in lower or "设置" in raw:
            ...
            profile = update_profile(chat_id or "local", updates, username)
            return "画像已更新。\n" + format_profile(chat_id or "local")
        return format_profile(chat_id or "local")
    ...
    return run_recommend_react_agent(raw, chat_id or "local", username, progress_callback=progress_callback).get("message", "")
