...
def _guess_intent(text: str) -> str:
    ...
    if any(token in raw for token in ("政策偏好", "国家政策", "最近政策")):
        return "policy"
    if any(token in raw for token in ("龙虎榜", "涨停板", "跌停板", "炸板", "板块热度", "筹码峰", "筹码分布", "涨停质量", "板质量", "封板质量", "晋级率", "晋级淘汰")):
        return "market_data"
...
def recommend_get_market_topic(topic: str = "report", trade_date: str = "") -> str:
    """读取宏观报告指定主题；topic 可为 lhb/limit_up/limit_quality/promotion/limit_down/broken_limit/strong/sector/report。"""
...
- 用户问“今天龙虎榜/涨停板/涨停质量/板质量/封板质量/晋级率/跌停板/炸板/强势股池/板块热度”时调用 recommend_get_market_topic。
