...
def create_order(data: AgentOrder, conn: sqlite3.Connection) -> int:
    ...
    c.execute(
        """INSERT INTO agent_order (agent_id, ts_code, stock_name, direction, order_type, quantity, price,
           open_get_in, reserved_cash, decision_batch_id, fill_probability, price_aggressiveness,
        ...
        (data.agent_id, data.ts_code, data.stock_name, data.direction, data.order_type,
         data.quantity, data.price, 1 if data.open_get_in else 0,
         data.reserved_cash, data.decision_batch_id, data.fill_probability, data.price_aggressiveness,
