...
def is_main_board(ts_code: str, name: str = "", market: str = "", status: str = "") -> bool:
...
def is_limit_up(pct_chg: float) -> bool:
    """是否涨停（>= 9.9%）"""
    return pct_chg >= 9.9


def is_limit_down(pct_chg: float) -> bool:
    """是否跌停（<= -9.9%）"""
    return pct_chg <= -9.9


def is_one_side_limit(open: float, high: float, low: float, close: float, pct_chg: float) -> bool:
...
def can_buy(ts_code: str, name: str, open: float, high: float, low: float,
    ...
    if is_one_side_limit(open, high, low, close, pct_chg):
